...
//  OpiaCompanion
//
//  v0.5 出门也能直接跟 Opia 聊 不切微信 不 ssh.
...
extension HorizontalAlignment {
    private struct OpiaBubbleEnd: AlignmentID {
        static func defaultValue(in d: ViewDimensions) -> CGFloat { d[.trailing] }
    }
    static let opiaBubbleEnd = HorizontalAlignment(OpiaBubbleEnd.self)
}

// MARK: - Brand defaults

let OpiaDefaultAIName = "Claude"
// Phase F 2026-05-11 — cccompanion default user name 改 "User" (4 letters 开源用户友好)
let OpiaDefaultUserName = "User"

// MARK: - Opia brand colors (dynamic light + dark) — internal 让其它 view 也能用

enum OpiaTheme: String, CaseIterable, Identifiable {
    case warm
    case terminal
...
enum OpiaColorSchemePref: String, CaseIterable, Identifiable {
    case system
    case light
    case dark
    var id: String { rawValue }
...
final class ThemeStore: ObservableObject {
    ...
    @Published var theme: OpiaTheme {
        didSet { UserDefaults.standard.set(theme.rawValue, forKey: "opia.theme") }
    }
    @Published var schemePref: OpiaColorSchemePref {
        didSet { UserDefaults.standard.set(schemePref.rawValue, forKey: "opia.colorScheme") }
    }
    ...
    @Published var followSystemColorScheme: Bool {
        didSet { UserDefaults.standard.set(followSystemColorScheme, forKey: "opia.followSystemColorScheme") }
    }
    private init() {
        let raw = UserDefaults.standard.string(forKey: "opia.theme") ?? OpiaTheme.warm.rawValue
        self.theme = OpiaTheme(rawValue: raw) ?? .warm
        let sRaw = UserDefaults.standard.string(forKey: "opia.colorScheme") ?? OpiaColorSchemePref.system.rawValue
        self.schemePref = OpiaColorSchemePref(rawValue: sRaw) ?? .system
        self.followSystemColorScheme =
            UserDefaults.standard.object(forKey: "opia.followSystemColorScheme") as? Bool ?? true
    }
...
extension Color {
    ...
    // environment binding.
    static var opiaFloatingBarBg: Color {
        switch ThemeStore.shared.theme {
        case .terminal: return termCard
        case .night: return nightCard
        case .warm: return warmCard
    ...
    static var opiaFloatingBarText: Color {
        switch ThemeStore.shared.theme {
        case .terminal, .night: return opiaText
        case .warm: return warmText
        }
    }
    static var opiaAssistant: Color {
        switch ThemeStore.shared.theme {
    ...
    static var opiaAssistantText: Color {
        switch ThemeStore.shared.theme {
...
extension Font {
    static func opiaBody(theme: OpiaTheme) -> Font {
...
nonisolated struct ChatMessage: Identifiable, Codable, Hashable, Sendable {
    ...
    func attachmentFullURL() -> URL? {
        ...
        return URL(string: OpiaServerConfig.serverURL.absoluteString + path)
    }
...
actor ChatNetworkClient {
    ...
    func fetchPoll(since: String?, etag: String?) async throws -> ChatPollResponse {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/poll")
...
final class ChatViewModel: ObservableObject {
    ...
    @Published var isOpiaTyping: Bool = false
    @Published var opiaStatus: String = "online"  // online | typing | sleeping
    ...
    var connectionStatus: ChatConnectionStatus {
        if opiaStatus == "typing" || isOpiaTyping { return .thinking }
    ...
    func start() {
        ...
        pollingTask = Task { [weak self] in
            ...
            let notifTask = Task { [weak self] in
                let stream = NotificationCenter.default.notifications(named: NSNotification.Name("OpiaResyncHistory"))
    ...
    private func pollOnce() async {
        do {
            ...
            isOpiaTyping = response.status.isTyping ?? (response.status.status == "typing")
            opiaStatus = response.status.status
    ...
    private func fetchSoundSettings() async {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("settings")
        do {
    ...
    private func refreshRecent() async {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/history")
    ...
    private func fetchTyping() async {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/typing")
        do {
            let (data, _) = try await session.data(from: url)
            if let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               let isTyping = obj["is_typing"] as? Bool {
                self.isOpiaTyping = isTyping
            }
        } catch {
            // 网络抖动静默
        }
    ...
    private func fetchStatus() async {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/status")
        do {
            let (data, _) = try await session.data(from: url)
            if let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               let status = obj["status"] as? String {
                self.opiaStatus = status
            }
        } catch {
            // 静默
        }
    ...
    func react(_ msg: ChatMessage, emoji: String) async {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/react")
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        if let s = OpiaServerConfig.sharedSecret, !s.isEmpty { req.setValue(s, forHTTPHeaderField: "X-Auth-Token") }
    ...
    private func runUpload(id: UUID) async {
        ...
        var components = URLComponents(
            url: OpiaServerConfig.serverURL.appendingPathComponent("chat/upload"),
    ...
    func searchServer() async {
        ...
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/search")
        var components = URLComponents(url: url, resolvingAgainstBaseURL: false)
        components?.queryItems = [
    ...
    var isOpiaWorking: Bool {
        if sending { return true }
        if opiaStatus == "working" || opiaStatus == "typing" { return true }
    ...
    func abortChain(session sessionName: String = "opia") async {
        print("[Opia abort] sending /chain/abort POST session=\(sessionName)")
        ...
        do {
            ...
            print("[Opia abort] response status=\(status) body=\(body)")
            if status != 200 {
                self.lastError = "中断失败: status \(status) \(body)"
            }
        } catch {
            print("[Opia abort] error: \(error)")
    ...
    func saveSelectedAsImage() async {
        ...
        #if canImport(UIKit)
        let aiName = OpiaNameResolver.name(for: .ai)
    ...
    func unfavoriteTurn(endingAt ts: String) async {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("favorites/delete_by_turn")
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        if let s = OpiaServerConfig.sharedSecret, !s.isEmpty {
    ...
    func addManyToFavorites(_ msgs: [ChatMessage]) async {
        ...
        let url = OpiaServerConfig.serverURL.appendingPathComponent("favorites/add")
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
    ...
    private func deleteOnServer(ts: String) async {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/delete")
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        if let s = OpiaServerConfig.sharedSecret, !s.isEmpty { req.setValue(s, forHTTPHeaderField: "X-Auth-Token") }
    ...
    private static func displayName(for role: String) -> String {
        return OpiaNameResolver.name(forMessageRole: role)
    }
    ...
    func regenerate(messageTs: String, extraReplaceIds: [String] = []) async {
        ...
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/regenerate")
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        if let s = OpiaServerConfig.sharedSecret, !s.isEmpty { req.setValue(s, forHTTPHeaderField: "X-Auth-Token") }
    ...
    private func postChatSendOptimistic(text: String, quotedTs: String?, replacing optimisticId: String) async -> Bool {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("chat/send")
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        if let secret = OpiaServerConfig.sharedSecret, !secret.isEmpty {
    ...
    func handleSlashCommand(_ text: String) async -> Bool {
        ...
        case "/stop":
            ...
            // (build 199 fix: /switch 之后 /stop 不再固定 abort opia)
        ...
        case "/switch":
            ...
            if sid.isEmpty {
                appendCommandReply("/switch 需要 sid\n用法 /switch <sid>\n常见 sid: opia / bao / shu / opus47-fresh / xtoken / awen / opia-female")
            } else {
    ...
    private func handleSlashCompact() async {
        // build 199 fix: /compact 跟随 active session (此前 server 端默认压 opia)
    ...
    func clearUnread() async {
        let url = OpiaServerConfig.serverURL.appendingPathComponent("push/clear-unread")
        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        if let s = OpiaServerConfig.sharedSecret, !s.isEmpty { req.setValue(s, forHTTPHeaderField: "X-Auth-Token") }
    ...
    func addTodo(_ text: String) async {
        // 不直接 POST /todos/add 改为让 Opia chain 接管 (2026-05-03 用户 push)
                ...
                onFavoriteTurn: vm.assistantTurnEndsTs.contains(msg.ts)
                    ? {
                        if FavoritedTurnsCache.shared.contains(msg.ts) {
                            ...
                            Task { await vm.unfavoriteTurn(endingAt: msg.ts); OpiaToastBus.shared.show("已取消收藏") }
                        } else {
                            FavoritedTurnsCache.shared.insert(msg.ts)
                            Task { await vm.addManyToFavorites(vm.turnMessages(endingAt: msg.ts)); OpiaToastBus.shared.show("已收藏") }
                        }
                    }
                    : nil,
            ...
            HStack(spacing: 12) {
                ...
                if vm.multiSelectMode {
                    Text("已选 \(vm.selectedTs.count) 条")
                        .font(.opiaSerifAdaptive(size: chatBodySize))
...
private struct MultiSelectActionBar: View {
    ...
    private func actionButton(
        ...
        Button(role: role, action: action) {
            ...
            .foregroundStyle(disabled ? Color.opiaTextDim.opacity(0.45) : Color.opiaText)
        }
        .disabled(disabled)
        .buttonStyle(.plain)
    }
...
private struct BackfillProgressBar: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 8)
        .background(Color.opiaCard.opacity(0.95))
    }
}
                ...
                HStack(spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text(item.displayName)
                            .font(.opiaSerifAdaptive(size: 12, weight: .medium))
                            .lineLimit(1)
...
private struct ChatInputBar: View {
    ...
    private var inputBarHStack: some View {
        HStack(spacing: 8) {
                ...
                if delayedIsWorking {
                    ...
                    Button {
                        ...
                        Image(systemName: "stop.circle.fill")
                            .font(.opiaSerifAdaptive(size: 18))
                    ...
                    Button {
                        ...
                        Image(systemName: speech.isRecording ? "mic.fill" : "mic")
                            .font(.opiaSerifAdaptive(size: 17))
                            .foregroundStyle(speech.isRecording ? Color.red : Color.opiaTextDim)
                            .padding(.trailing, 10)
                    }
            ...
            Button { commitFromSendButton() } label: {
                Image(systemName: (vm.sending || commitPending) ? "ellipsis.circle" : "paperplane.fill")
                    ...
                    .foregroundStyle(hasContent ? Color.opiaAssistant : Color.opiaTextDim)
            }
        ...
        .onChange(of: vm.isOpiaWorking) { _, working in
            if working {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    if vm.isOpiaWorking { delayedIsWorking = true }
                }
            } else {
                delayedIsWorking = false
            }
        }
        .onReceive(NotificationCenter.default.publisher(for: .opiaPasteFromClipboard)) { _ in
            pasteFromClipboard()
        }
    }
                                ...
                                Button {
                                    ...
                                    HStack(spacing: 2) {
                                        Image(systemName: "pencil")
                                            .font(.opiaSerifAdaptive(size: 9, weight: .semibold))
                                        Text("编辑")
                                            .font(.opiaSerifAdaptive(size: 10, weight: .medium))
                                    }
...
private struct ChatToolbarLeading: View {
    ...
    private var avatarView: some View {
        OpiaAvatarView(role: .ai, size: 38)
    }
    ...
    var body: some View {
        if multiSelectMode {
            ...
            HStack(spacing: 8) {
                ...
                Text(aiName)
                    .font(.opiaSerifAdaptive(size: 17, weight: .semibold))
                    // 2026-05-14 build 198 — opiaAssistant 在 terminal theme = Color.clear 导致 AI 名字看不见
                    // 改用 opiaAccent (每个主题都有可见 accent: warm 橙 / terminal 浅青 / night 暖橙)
                    .foregroundStyle(Color.opiaAccent)
            }
...
private struct ChatToolbarPrincipal: View {
    ...
    var body: some View {
        if multiSelectMode {
            ...
            HStack(spacing: 6) {
                Text(aiAvatarEmoji)
                    .font(.opiaSerifAdaptive(size: 22, weight: .bold))
                Text(aiName)
                    .font(.opiaSerifAdaptive(size: 20, weight: .semibold))
                    .foregroundStyle(Color.opiaText)
            }
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(message.isUser ? "回复你自己" : "回复 Opia")
                    .font(.opiaSerifAdaptive(size: 11, weight: .semibold))
                    .foregroundStyle(Color.opiaAccent)
                Text(message.text)
                    .font(.opiaSerifAdaptive(size: 12))
                    .foregroundStyle(Color.opiaTextDim)
        ...
        .padding(.vertical, 8)
        .background(Color.opiaCard)
        .overlay(Rectangle().frame(width: 2).foregroundStyle(Color.opiaAccent), alignment: .leading)
    }
}
    ...
    var body: some View {
        ScrollViewReader { proxy in
            ZStack(alignment: .bottomTrailing) {
                ...
                // Phase F (item 2) — bg image 存在时走 clear 透出 image, 否则走 opiaBg
                .background(chatBackgroundPath.isEmpty ? Color.opiaBg : Color.clear)
                ...
                .onScrollGeometryChange(for: Bool.self) { geo in
                    ...
                    // 2026-05-14 build 189 — isOpiaTyping 切换时 TypingStatusBar 出现/消失会瞬间改变
                    // contentSize/containerSize, 触发本回调而非用户真滑. 在 typing 状态下跳过更新, 等 typing
                    // 结束后状态会通过下一次真正滑动 reset.
                    guard !vm.isOpiaTyping else { return }
                    ...
                    Button {
                        ...
                        HStack(spacing: 4) {
                            Image(systemName: "chevron.down")
                                .font(.opiaSerifAdaptive(size: 11, weight: .bold))
            ...
            HStack(spacing: 6) {
                ForEach(ChatViewModel.SearchFilter.allCases) { filter in
                    Button {
                        ...
                        Text(filter.rawValue)
                            ...
                            .foregroundStyle(selected == filter ? Color.white : Color.opiaText)
                            .padding(.horizontal, 14)
                            .padding(.vertical, 6)
                            .background(
                                Capsule()
                                    .fill(selected == filter ? Color.opiaAssistant : Color.opiaCard)
                            )
                    }
                    .buttonStyle(.plain)
                }
                if let onDate {
                    Button {
                        ...
                        .padding(.horizontal, 12)
                        .padding(.vertical, 6)
                        .background(Capsule().fill(Color.opiaCard))
                    }
                    .buttonStyle(.plain)
                }
            }
...
private struct TimelineNodeRow<Content: View>: View {
    ...
    private var node: some View {
        Circle()
            ...
            .overlay(Circle().stroke(Color.opiaBg, lineWidth: 2))
    }

    private var color: Color {
        switch kind {
        case .user: return Color.opiaAccent
        case .opia: return Color.opiaAssistant
        case .task: return Color.opiaTextDim.opacity(0.55)
        case .separator: return Color.opiaTextDim.opacity(0.4)
        case .other(let role): return Self.hashColor(role)
        }
    }

    private var lineColor: Color {
        Color.opiaTextDim.opacity(0.25)
    }
...
private func timelineKind(for msg: ChatMessage) -> TimelineNodeKind {
    ...
    if msg.role == "assistant" { return .opia }
    ...
    var body: some View {
        HStack {
            ...
            Text(label)
                ...
                .padding(.horizontal, 10)
                .padding(.vertical, 3)
                .background(Color.opiaCard.opacity(0.7))
                .clipShape(Capsule())
            Spacer()
        }
...
private struct ToolActivityStackView: View {
    ...
    private var runningView: some View {
        ...
        .padding(.horizontal, 10)
        .padding(.vertical, 6)
        .background(Color.opiaCard.opacity(0.5))
        .clipShape(Capsule())
        .frame(maxWidth: .infinity, alignment: .leading)
        .onAppear {
            pulseScale = 1.1
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    ...
                    Image(systemName: "wrench.and.screwdriver")
                        ...
                        .foregroundStyle(Color.opiaAccent)
                    Text(stack.summary)
                        .font(.opiaSerifAdaptive(size: 11, weight: .medium))
                        .foregroundStyle(Color.opiaTextDim)
        ...
        HStack(spacing: 8) {
            if multiSelectMode {
                Image(systemName: selected ? "checkmark.circle.fill" : "circle")
                    .font(.opiaSerifAdaptive(size: 20, weight: .semibold))
                    .foregroundStyle(selected ? Color.opiaAssistant : Color.opiaTextDim)
                    .frame(width: 28)
            }
            ChatBubble(
                message: message,
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            Text(typingVerbsEnabled
                ...
                .font(.opiaSerifAdaptive(size: 12))
                // 2026-05-14 build 198 — terminal 主题下用 opiaAccent (浅青) 比 opiaText (白) 跟 dark
        ...
        Button(action: {
            ...
            HStack(spacing: 8) {
                Image(systemName: iconName)
                    ...
                    .foregroundStyle(Color.white)
                Text(label)
                    .font(.opiaSerifAdaptive(size: 16, weight: .medium))
                    .foregroundStyle(Color.white)
                    .lineLimit(1)
                Spacer(minLength: 0)
        ...
        HStack(alignment: .center, spacing: 12) {
            ...
            ZStack {
                RoundedRectangle(cornerRadius: 8, style: .continuous)
                    .fill(Color.opiaCard)
                OpiaAvatarView(role: message.isUser ? .user : .ai, size: 34)
            }
            .frame(width: 40, height: 40)

            VStack(alignment: .leading, spacing: 3) {
                HStack(alignment: .firstTextBaseline) {
                    Text(message.isUser ? OpiaNameResolver.name(for: .user) : OpiaNameResolver.name(for: .ai))
    ...
    private var highlightedSnippet: Text {
        ...
        if let range = text.range(of: needle, options: .caseInsensitive) {
            ...
            return Text(beforeKw)
                + Text(kw).foregroundStyle(Color.opiaAccent).bold()
                + Text(afterKw)
        }
        return Text(text)
    }
                    ...
                    HStack(alignment: .top, spacing: 0) {
                        ...
                        VStack(alignment: msg.isUser ? .trailing : .leading, spacing: 4) {
                            Text(msg.isUser ? "眠" : aiName)
                                .font(.opiaSerifAdaptive(size: 10, weight: .medium))
                                .foregroundStyle(Color.opiaTextDim.opacity(0.8))
                                .padding(.horizontal, 4)

                            Text(msg.text)
                                .font(.opiaSerifAdaptive(size: 14))
                                .foregroundStyle(msg.isUser ? Color.opiaUserText : Color.opiaAssistantText)
                                .padding(.horizontal, 12)
                                .padding(.vertical, 10)
                                .background(msg.isUser ? Color.opiaUser : Color.opiaAssistant)
...
struct ChatBubble: View {
        ...
        HStack(alignment: .bottom, spacing: 0) {
            ...
            VStack(alignment: message.isUser ? .trailing : .leading, spacing: 2) {
                        ...
                        Button {
                            ...
                            HStack(spacing: 8) {
                                if isDownloadingPreview {
                                    ...
                                    ZStack {
                                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                                            .fill(Color.opiaAccent.opacity(0.18))
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 12, style: .continuous)
                                    .stroke(Color.opiaAccent.opacity(0.15), lineWidth: 0.5)
                            )
                        }
                        .buttonStyle(.plain)
                ...
                if !message.text.isEmpty {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ForEach(Array(segments.enumerated()), id: \.offset) { _, seg in
                            ...
                            case .text(let s):
                                ...
                                Text(s)
                                    .font(.opiaSerifAdaptive(size: chatBodySize))
                                    .foregroundStyle(message.isUser ? Color.opiaUserText : Color.opiaAssistantText)
                    ...
                    .padding(.horizontal, 12)
                    .padding(.vertical, 8)
                    .background(message.isUser ? Color.opiaUser : Color.opiaAssistant)
                    .foregroundStyle(message.isUser ? Color.opiaUserText : Color.opiaAssistantText)
                    .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))
                }
                ...
                if let reactions = message.reactions, !reactions.isEmpty {
                    HStack(spacing: 4) {
                        ForEach(Array(Set(reactions)), id: \.self) { emoji in
                            ...
                            HStack(spacing: 2) {
                                Text(emoji).font(.opiaSerifAdaptive(size: 12))
                                if count > 1 {
                                    Text("\(count)")
                                        .font(.opiaSerifAdaptive(size: 11))
                                        .foregroundStyle(Color.opiaTextDim)
                        ...
                        if !message.isUser, let fn = onFavoriteTurn {
                            Button(action: fn) {
                                ...
                                Image(systemName: favorited ? "bookmark.fill" : "bookmark")
                                    .font(.system(size: 12))
                                    .foregroundStyle(favorited ? Color.opiaAccent : Color.opiaTextDim)
                            }
                            .buttonStyle(.plain)
                        }
                    }
    ...
    private func choiceButtons(_ options: [ChoiceOption]) -> some View {
        ...
        VStack(spacing: 0) {
            ForEach(Array(options.enumerated()), id: \.offset) { idx, opt in
                if idx > 0 {
                    Divider()
                        .background(Color.opiaAssistant.opacity(0.18))
                }
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .stroke(Color.opiaAssistant.opacity(0.25), lineWidth: 0.5)
        )
        .clipShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
    }
        ...
        Button {
            ...
            HStack(spacing: 10) {
                ...
                Text(String(UnicodeScalar(0x41 + min(index, 25))!))
                    ...
                    .background(Circle().fill(Color.opiaAssistant.opacity(0.14)))
                Text(opt.label)
                    .font(.system(.callout, design: .serif))  // New York 衬线 偏手帐
                    .foregroundStyle(Color.opiaAssistant)  // 橘红 不再黑
                    .multilineTextAlignment(.leading)
                    .frame(maxWidth: .infinity, alignment: .leading)
                Image(systemName: "chevron.right")
                    .font(.opiaSerifAdaptive(size: 11))
    ...
    private func downloadToCache(remoteURL: URL, filename: String) async -> URL? {
        ...
        let previewDir = cachesDir.appendingPathComponent("opia_preview", isDirectory: true)
...
struct PhotoEditView: View {
        ...
        Button {
            ...
            VStack(spacing: 3) {
                Image(systemName: UIImage(systemName: icon) != nil ? icon : (fallback ?? icon))
                    .font(.opiaSerifAdaptive(size: 20, weight: .semibold))
                Text(label).font(.opiaSerifAdaptive(size: 11))
            }
            .foregroundStyle(mode == target ? Color.opiaAccent : Color.white)
            .frame(maxWidth: .infinity)
            .padding(.vertical, 4)
        }
        .buttonStyle(.plain)
    ...
    private func modeControlBar() -> some View {
        if mode == .draw {
                ...
                ForEach(PhotoEditView.drawWidths, id: \.self) { w in
                    Button {
                        ...
                        Circle()
                            ...
                            .overlay(
                                Circle().stroke(selectedDrawWidth == w ? Color.opiaAccent : Color.clear, lineWidth: 2)
                            )
                    }
                    .buttonStyle(.plain)
                }
            ...
            HStack(spacing: 12) {
                Text("马赛克").font(.opiaSerifAdaptive(size: 12)).foregroundStyle(.white.opacity(0.7))
                ForEach(0..<4, id: \.self) { lvl in
                    Button {
                        ...
                        Text(lvl == 0 ? "关" : "\(Int(PhotoEditView.mosaicScales[lvl]))")
                            ...
                            .padding(.horizontal, 10)
                            .padding(.vertical, 4)
                            .background(mosaicLevel == lvl ? Color.opiaAccent : Color.white.opacity(0.15))
                            .foregroundStyle(.white)
                            .clipShape(Capsule())
                    }
                    .buttonStyle(.plain)
            ...
            HStack(spacing: 8) {
                Text("点图加文字 长按拖动").font(.opiaSerifAdaptive(size: 12)).foregroundStyle(.white.opacity(0.7))
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 6)
        } else {
...
private struct SelectableUITextWrapper: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> IntrinsicSelectableTextView {
        ...
        tv.tintColor = UIColor(Color.opiaAccent)
        tv.linkTextAttributes = [
            .foregroundColor: UIColor(Color.opiaAccent),
...
struct SearchStateContent: View {
    ...
    private func sectionHeader(_ title: String) -> some View {
        Text(title)
            .font(.opiaSerifAdaptive(size: 11))
            .foregroundStyle(Color.opiaTextDim.opacity(0.6))
            .textCase(.uppercase)
            .padding(.leading, 16)
            .padding(.top, 12)
            .padding(.bottom, 4)
        ...
        VStack(spacing: 10) {
            if showProgress {
                ...
                Image(systemName: icon)
                    .font(.opiaSerifAdaptive(size: 28))
                    .foregroundStyle(Color.opiaTextDim.opacity(0.6))
            }
            Text(title)
                .font(.opiaSerifAdaptive(size: 14))
        ...
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 8, style: .continuous)
                    .fill(Color.opiaCard)
                Image(systemName: icon)
                    .font(.opiaSerifAdaptive(size: 20))
                    .foregroundStyle(Color.opiaAccent)
            }
            .frame(width: 40, height: 40)
            VStack(alignment: .leading, spacing: 3) {
...
struct LinkResultRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text(host)
                    .font(.opiaSerifAdaptive(size: 13, weight: .semibold))
                    .foregroundStyle(Color.opiaAccent)
                    .lineLimit(1)
                Text(title)
                    .font(.opiaSerifAdaptive(size: 14))
...
struct AudioResultRow: View {
    ...
    private var sender: String {
        message.isUser ? OpiaNameResolver.name(for: .user) : OpiaNameResolver.name(for: .ai)
    }
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 8) {
                    Text(durationText)
                        .font(.opiaSerifAdaptive(size: 13, weight: .medium))
                        .foregroundStyle(Color.opiaAccent)
        ...
        ZStack {
            ...
            if !path.isEmpty, let ui = UIImage(contentsOfFile: path) {
                ...
                Image(systemName: "person.crop.circle.fill")
                    .font(.opiaSerifAdaptive(size: size - 2, weight: .bold))
                    .foregroundStyle(Color.white)
                    .frame(width: size, height: size)
            } else {
                Text(aiAvatarEmoji)
                    .font(.opiaSerifAdaptive(size: size - 4))
                    .frame(width: size, height: size)
